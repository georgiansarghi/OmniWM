...
| `autoHide` | boolean | `false` | **Unreleased (build from `main`):** reveals immediately near the bar/its edge segment; hides immediately when the pointer leaves its keep-open area. Both delays are zero. Keeps open for related popups and window-list sheets. Overlay-only: never reserves layout space. Supports per-display overrides. |
