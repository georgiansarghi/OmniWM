...
| `autoHide` | boolean | `false` | **Unreleased (build from `main`):** reveals immediately near the bar/its edge segment; hides immediately when the pointer leaves its keep-open area unless an activity reveal is active. Both hover delays are zero. Keeps open for related popups and window-list sheets. Overlay-only: never reserves layout space. Supports per-display overrides. |
| `activityReveal` | string | `"off"` | **Unreleased:** requires auto-hide. `"off"`, `"workspace"`, `"workspaceAndColumn"` (Niri), or `"focus"` (managed focused-window changes). All enabled presets include workspace changes. Supports per-display overrides. |
