...
- **Auto-hide** (unreleased, source builds): set `autoHide = true` under `[workspaceBar]` or enable **Auto-Hide on Pointer Leave** globally/per display. Both hover delays are zero. Popups and grouped-window sheets keep it open. Auto-hide never reserves layout space, and modifier reveal can be used as an alternative. Manual hiding and native-fullscreen suppression take precedence.
- **Activity reveal** (unreleased, source builds): with auto-hide, optionally set **Briefly Show After Changes** to workspace, workspace + Niri column, or managed focus changes. Defaults to off. For navigation feedback, use `activityReveal = "workspaceAndColumn"` and `activityRevealSeconds = 1.0`. The affected bar appears immediately and stays visible until one second after the last change (or longer while hovered/using popups), without moving windows. Both settings support per-display overrides.
