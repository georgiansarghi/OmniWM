...
- **Visibility and reveal triggers** (unreleased, source builds): choose **Always Visible** or **Show Temporarily** globally/per display (`visibility = "alwaysVisible"` / `"temporary"`). Temporary bars have independent pointer, activity, and modifier triggers and never reserve layout space. Set `revealOnHover = false` to prevent the mouse from summoning the bar; hovering an already-visible bar or using its popups still keeps it open. Hover delays stay zero. Manual hiding and native-fullscreen suppression take precedence. Legacy `autoHide`/modifier-only configurations preserve their behavior on load.
- **Activity-only reveal** (unreleased, source builds): use `visibility = "temporary"`, `revealOnHover = false`, `activityReveal = "workspaceAndColumn"`, and `activityRevealSeconds = 1.0`. The affected bar appears immediately on navigation and remains until one second after the last change, or longer during interaction, without moving windows. Activity defaults to off; all four settings support per-display overrides. Always Visible ignores reveal triggers without discarding preferences.
- **Bottom / Left / Right placement** (unreleased, source builds) anchors the bar to the usable display edge, avoiding a visible Dock. Left/right bars stack content vertically and scroll when needed; **Bar Thickness** (`height` in TOML) controls their width. Layout space is reserved at the selected edge, and popups open inward from displayed bounds. These positions support per-display overrides and ignore saved notch modes.
...
The unreleased placement/visibility/activity features above are in [this fork's PR #1](https://github.com/georgiansarghi/OmniWM/pull/1), branch `georgiansarghi/OmniWM:feat/workspace-bar-bottom`, not upstream `main`.
